...
import com.mycompany.data.Generator;
...
public class ReportsView extends HorizontalLayout implements View {
    ...
    private HorizontalLayout createEditorInstance(int which,
        ...
        if (!helpShown) {
            helpShown = true;
            HelpManager helpManager = ((MyVaadinUI) getUI()).getHelpManager();
        ...
        ((MyVaadinUI) UI.getCurrent()).updateReportsButtonBadge(draftCount
                + "");
    ...
    private AbstractComponent createComponentFromPaletteItem(String caption,
        ...
        if (caption != null) {
            if (caption.equals("text")) {
                ...
                t.setContainerDataSource(((MyVaadinUI) getUI()).dataProvider
                        .getRevenueByTitle());
                t.setWidth("100%");
                t.setPageLength(0);
